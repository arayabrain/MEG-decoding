...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        B, _, L = x.shape
        h = self.group_norm(x)
        h = h.swapaxes(1, 2)  # [B, C, L] --> [B, L, C]
        h, _ = self.mhsa(h, h, h)  # [B, L, C]
        h = h.swapaxes(2, 1)  # [B, L, C] --> [B, C, L]
        return x + h
...
class UNet(nn.Module):
    def __init__(
        ...
        apply_attention=(True, True, True, True), # (False, False, True, False), # embeddingはconvが意味なさそうなので
        dropout_rate=0.1,
        time_multiple=4,
    ):
        super().__init__()
